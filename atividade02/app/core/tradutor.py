...
def texto_para_morse(text: str) -> str:
    ...
    # Limpa o texto: remove espaços extras e converte para maiúsculas
    texto_limpo = ' '.join(text.upper().split())

    # Converte cada caractere para Morse, usando '?' para caracteres desconhecidos
    return ' '.join(config.MORSE_MAP.get(char, '?') for char in texto_limpo)
...
def morse_para_texto(morse: str) -> str:
    ...
    return ' '.join(
        ''.join(config.REVERSE_MORSE_MAP.get(letra, '?') for letra in palavra.split())
        for palavra in morse.strip().split(' / ')
    )